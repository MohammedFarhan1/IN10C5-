...
import { Download, Printer, QrCode, Search, Trash2 } from 'lucide-react';
import toast from 'react-hot-toast';
import Link from 'next/link';
import { deleteUnitPassport } from '@/app/actions/seller';
...
export default function SellerUnitClient({ initialUnits }: { initialUnits: UnitRow[] }) {
  ...
  const filteredUnits = units.filter((u) => {
    const code = (u.unique_unit_code || '').toLowerCase();
    const serial = (u.serial_number || '').toLowerCase();
    const query = searchQuery.toLowerCase();
    return code.includes(query) || serial.includes(query);
  });
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative w-full max-w-md">
            <input
              type="text"
              placeholder="Search serial number or unit code..."
              value={searchQuery}
          ...
          <span className="text-xs text-gray-400 font-mono">
            {filteredUnits.length} unit tokens
          </span>
        </div>
              ...
              {filteredUnits.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-400">
                    No unit passports match the current filter.
                  </td>
                </tr>
              ) : (
                filteredUnits.map((u) => {
                    ...
                    <tr key={u.id || u.unique_unit_code} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            onClick={async () => {
                              if (!confirm('Completely delete this unit passport? All associated scan records will be wiped permanently.')) return;
                              try {
                                const res = await deleteUnitPassport(u.unique_unit_code);
                                if (res.success) {
                                  toast.success('Deleted unit passport completely.');
                                  setUnits((prev) => prev.filter((item) => item.unique_unit_code !== u.unique_unit_code));
                                } else {
                                  toast.error(res.error || 'Deletion failed.');
                                }
                              } catch {
                                toast.error('Request failed during deletion.');
                              }
                            }}
                            title="Delete Unit Passport"
                            className="p-1.5 bg-red-50 hover:bg-red-100 text-red-500 rounded-xl border border-red-100 font-bold transition flex items-center justify-center"
                          >
          ...
          <div className="bg-white border border-gray-200 rounded-3xl p-6 md:p-8 max-w-md w-full space-y-6 relative shadow-2xl">
            ...
            <div className="text-center">
              <span className="text-[10px] text-red-600 font-mono uppercase bg-red-50 border border-red-100 px-2 py-0.5 rounded-full font-bold">
                Delivery Authenticity Certificate
              </span>
            ...
            <div className="bg-white p-6 rounded-2xl flex flex-col items-center justify-center shadow-inner relative border-4 border-gray-100">
              ...
              <span className="text-[9px] font-mono text-gray-400 uppercase tracking-widest mt-3 block text-center">
                Scan using smartphone to open verification passport
              </span>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                download={`LABEL_${selectedUnitForQr.unique_unit_code}.png`}
